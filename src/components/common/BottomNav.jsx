...
export default function BottomTab() {
  ...
  const Item = ({ to, active, children, isCreate }) => (
    <Link
      to={to}
      className={`relative flex flex-col items-center justify-center flex-1 py-2 transition-all duration-300 ${isCreate ? "" : active ? "text-primary" : "text-base-content/60 hover:text-base-content"
        }`}
    >
  ...
  return (
    <div className="lg:hidden fixed bottom-0 left-0 right-0 z-40 border-t border-base-200 bg-base-100/95 backdrop-blur-xl shadow-[0_-4px_16px_rgba(0,0,0,0.06)]">
      <div className="max-w-2xl mx-auto h-16 px-4 flex items-center justify-around">
        ...
        <Link
          to="/create-reel"
          className="relative flex items-center justify-center w-12 h-12 -mt-6 rounded-2xl bg-gradient-to-br from-primary via-secondary to-accent shadow-lg hover:shadow-xl transition-all duration-300 hover:scale-105 active:scale-95"
        >
          <div className="absolute inset-0 rounded-2xl bg-gradient-to-br from-primary via-secondary to-accent blur opacity-50" />
          <Plus size={28} className="relative text-primary-content" strokeWidth={2.5} />
        </Link>
